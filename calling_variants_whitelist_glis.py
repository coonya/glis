...
def change_line(line):
	...
	x_2 = x_1[7].split(';')
	DP = x_2[0].replace('DP=','')
	DP4 = x_1[7].split('DP4=')[1].split(';')[0].split(',')
	print DP4
	ref_allele = str(int(DP4[0]) + int(DP4[1]))
	var_allele = str(int(DP4[2]) + int(DP4[3]))
	total_allele = str(int(DP4[0]) + int(DP4[1]) + int(DP4[2]) + int(DP4[3]))

	x_1[8] = 'GT:DP:AD'
	x_1[9] = '0/1:%s:%s,%s' % (total_allele, ref_allele, var_allele)
...
samtools_path = params['samtools1_path']
...
for y in open(output_maf).xreadlines():
	...
	if y[0:11] == 'Hugo_Symbol':
		...
		y_1[tumor_id] = y_1[tumor_id].split('_')[-1]

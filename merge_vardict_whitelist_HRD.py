...
parser.add_option('-r', dest='hrd', help='hrd list maf')
...
def __main__():
	...
	if options.hrd:	
		f1_size = os.path.getsize(options.hrd)
		
		if f1_size != 0:
			for z in open(options.hrd).xreadlines():
				z_1 = z.strip().split('\t')
				key3 = '%s_%s' % (z_1[4], z_1[5])
				if len(dic) == 1:
					wfile.write(dic['Chromosome_Start_Position'])
				else:
					if dic.has_key(key3):
						pass
					else:
	...
	f2_size = os.path.getsize(options.white)

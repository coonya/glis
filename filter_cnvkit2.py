...
def sigflag(gene, cn, diagnosis):
	#drug = {'ERBB2':'Amplification', 'CDK4':'Amplification', 'MET':'Amplification', 'FGFR1':'Amplification', 'FGFR2':'Amplification'}
	
	drug = {}
	for x in open(options.drug).xreadlines():
		x_1 = x.strip().split('\t')
		if x_1[2] == 'Amplification' or x_1[2] == 'Deletion':
			key1 = '%s_%s' % (x_1[1], x_1[3])
			drug[key1] = x
	...
	key2 = '%s_%s' % (gene, event)

	if drug.has_key(key2):
		val = 'Yes'
	else:
		val = 'No'
	return val

...
def __main__():
	...
	check_option(options.dbsnp_snv, "You need to use '-i' option with input vcf file\n")
	check_option(options.pon_snv, "You need to use '-i' option with input vcf file\n")
	#check_option(options.pon_indel, "You need to use '-i' option with input vcf file\n")
	check_option(options.KRG_common, "You need to use '-i' option with input vcf file\n")
	#check_option(options.KRG_indel, "You need to use '-i' option with input vcf file\n")
	check_option(options.KRG_rare, "You need to use '-i' option with input vcf file\n")
	#check_option(options.black, "You need to use '-i' option with input vcf file\n")
	...
	black_file = '/data/D161740/Reference/BlackList/non_filtered_20170111.sorted.txt'

	dbsnp_file = options.dbsnp_snv
	PON_file = options.pon_snv
	KRG_common_file = options.KRG_common
	KRG_rare_file = options.KRG_rare
	...
	print 'Complete reading black list file.'
	...
	"""
			...
			KRG_rare_com = db_com(dic_key, KRG_rare_dic)
			ExAC_com = exac(x_1[ExAC_AF_EAS_pos])
			black_com = db_com(dic_key, black)
			...
			x_1.append(KRG_rare_com)
			x_1.append(ExAC_com)
			x_1.append(black_com)

			if dbsnp_com == 'Yes' or PON_com == 'Yes' or KRG_common_com == 'Yes' or KRG_rare_com == 'Yes' or ExAC_com == 'Yes' or black_com == 'Yes':
				x_1.append('Yes')
				total_com = 'Yes'
			else:
				x_1.append('No')
